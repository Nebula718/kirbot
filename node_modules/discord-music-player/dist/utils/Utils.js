...
class Utils {
        ...
        try {
            ...
            if (SOptions?.sortBy !== null && SOptions?.sortBy !== 'relevance')
                Filters = Array.from((await ytsr_1.default.getFilters(Filters.url))
                    ...
                    ?? Filters;
            let Result = await ytsr_1.default(Filters.url, {
                limit: Limit,
            });
    ...
    static async link(Search, SOptions = __1.DefaultPlayOptions, Queue) {
        ...
        if (AppleLink) {
            try {
                let AppleResult = await AppleUtils_1.getSong(Search);
            ...
            try {
                let SpotifyResult = await spotify_url_info_1.getPreview(Search);
    ...
    static async playlist(Search, SOptions = __1.DefaultPlaylistOptions, Queue) {
        ...
        if (ApplePlaylistLink) {
            let AppleResultData = await AppleUtils_1.getPlaylist(Search).catch(() => null);
            if (!AppleResultData)
            ...
            let SpotifyResultData = await spotify_url_info_1.getData(Search).catch(() => null);
